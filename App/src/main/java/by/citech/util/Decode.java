...
//                      cNames.put((T) field.get(null), field.getName());
    ...
    private AudioRecord findAudioRecord(boolean debug) {
        ...
        if (debug) Log.i(Tags.FROM_MIC, "findAudioRecord");
                ...
                for (short channel : mAudioFormatChannels) {
                    try {
                        ...
                        if (bufferSizeMinimal != AudioRecord.ERROR_BAD_VALUE) {
                            ...
                            if (debug) Log.i(Tags.FROM_MIC, "findAudioRecord new AudioRecord");
                            Thread.sleep(100);
                            if (recorder.getState() == AudioRecord.STATE_INITIALIZED) {
                                Log.i(Tags.FROM_MIC, "findAudioRecord recorder is initialized");
                                bufferLenghtActual = bufferSizeMinimal * 10;
                                buffer = new byte[bufferLenghtActual];
                                return recorder;
                            }
                            if (debug) Log.i(Tags.FROM_MIC, "findAudioRecord recorder is not initialized, release");
                            recorder.release();
                            recorder = null;
                            Thread.sleep(100);
                        }
                    } catch (Exception e) {
                        if (debug) Log.i(Tags.FROM_MIC, "findAudioRecord Exception");
                        if (recorder != null) {
                            recorder.release();
                        }
                        recorder = null;
