...
public class ServerOffTask extends AsyncTask<Void, Void, Void> {
    ...
    public ServerOffTask(ServerActivity activity, NanoWebSocketServerCtrl serverCtrl) {
    ...
    protected Void doInBackground(Void... params) {
        Log.i(Tags.CLT_TASK_SRVOFF, "doInBackground");
...
//          while (webSocket.isOpen()) {
            while (!serverCtrl.getStatus().equals(StatusMessages.WEBSOCKET_CLOSED)
                ...
                try {
                    ...
                    Log.i(Tags.CLT_TASK_SRVOFF, "doInBackground InterruptedException - 1");
                }
            }

            Log.i(Tags.CLT_TASK_SRVOFF, "doInBackground socket closed");
        } else {
            Log.i(Tags.CLT_TASK_SRVOFF, "doInBackground socket null or already closed");
        }
        ...
        if (serverCtrl.isAlive()) {
            ...
            while (serverCtrl.isAlive()) {
                try {
                    ...
                    Log.i(Tags.CLT_TASK_SRVOFF, "doInBackground InterruptedException - 2");
                }
            }

            Log.i(Tags.CLT_TASK_SRVOFF, "doInBackground server stopped");
        } else {
            Log.i(Tags.CLT_TASK_SRVOFF, "doInBackground server already stopped");
        }
    ...
    protected void onPostExecute(Void aVoid) {
        ...
        Log.i(Tags.CLT_TASK_SRVOFF, "onProgressUpdate");
