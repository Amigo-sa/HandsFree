package by.citech.network.server.connection.protocols.http;
...
import by.citech.network.server.connection.protocols.http.NanoHTTPD.ResponseException;
import by.citech.network.server.connection.protocols.http.content.ContentType;
import by.citech.network.server.connection.protocols.http.content.CookieHandler;
import by.citech.network.server.connection.protocols.http.request.Method;
import by.citech.network.server.connection.protocols.http.response.Response;
import by.citech.network.server.connection.protocols.http.response.Status;
import by.citech.network.server.connection.protocols.http.tempfiles.ITempFile;
import by.citech.network.server.connection.protocols.http.tempfiles.ITempFileManager;
...
public class HTTPSession implements IHTTPSession {
        ...
        try {
            ...
            for (int boundaryIdx = 0; boundaryIdx < boundaryIdxs.length - 1; boundaryIdx++) {
                ...
                if (mpline == null || !mpline.contains(contentType.getBoundary())) {
                    throw new ResponseException(Status.BAD_REQUEST, "BAD REQUEST: Content type is multipart/form-data but chunk does not prepareStream with boundary.");
                }
    ...
    /**
     * Find the byte positions where multipart boundaries prepareStream. This reads a
    ...
    private int[] getBoundaryPositions(ByteBuffer b, byte[] boundary) {
        ...
        do {
            ...
            // Copy the end of the buffer to the prepareStream
