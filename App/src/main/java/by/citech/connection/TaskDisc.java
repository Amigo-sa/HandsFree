...
public class TaskDisc extends AsyncTask<IConnCtrl, Void, Void> {
    ...
    protected Void doInBackground(IConnCtrl... iConnCtrl) {
        if (Settings.debug) Log.i(Tags.TASK_DISC, "doInBackground");
        if (iConnCtrl[0].isAliveConnection()) {
            ...
            if (!iConnCtrl[0].isAliveConnection()) {
                if (Settings.debug) Log.i(Tags.TASK_DISC, "doInBackground connection closed");
                return null;
            }
            iConnCtrl[0].closeConnectionForce();
            if (Settings.debug) Log.e(Tags.TASK_DISC, "doInBackground connection closed force");
        }
        if (Settings.debug) Log.i(Tags.TASK_DISC, "doInBackground connection already closed");
        return null;
    }

    @Override
    protected void onPostExecute(Void aVoid) {
        Log.i(Tags.TASK_DISC, "onPostExecute");
        iDisc.disconnected();
    }
}
